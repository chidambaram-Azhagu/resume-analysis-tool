...
def extract_percentage_match_info(response):
    try:
        ...
        logging.error("Error in extract_percentage_match_info(): %s", e)
...
def extract_fit_for_role_response_info(response):
    try:
        ...
        logging.error("Error in extract_fit_for_role_response_info(): %s", e)
...
def get_gemini_response(job_description, pdf_content, prompt):
    try:
        ...
        logging.error("Error in get_gemini_response(): %s", e)
...
def get_gemini_response_summary(prompt, pdf_content):
    try:
        ...
        logging.error("Error in get_gemini_response_summary(): %s", e)
...
def input_pdf_text(uploaded_file):
    try:
        ...
        logging.error("Error in input_pdf_text(): %s", e)
...
def get_gemini_response_filter(input):
    try: 
        ...
        logging.error("Error in get_gemini_response_filter(): %s", e)
...
def parse_s3_url(s3_url):
    try:
        ...
        logging.error("Error in parse_s3_url(): %s", e)
...
def process_s3_input(s3_url):
    try:
        ...
        if download_file_from_s3(bucket_name, object_key, file_path):
            ...
            logging.error("Error in process_s3_input(): %s", e)
            return None
    except Exception as e:
        logging.error("Error processing S3 input: %s", e)
                ...
                if not uploaded_files:
                    ...
                    def process_file(uploaded_file):
                        try:
                            ...
                            logging.error("Error in summary_button data: %s", e)
                ...
                if not input_text:
                    ...
                    def process_file(uploaded_file):
                        try:
                            ...
                            logging.error("Error in submit data(): %s", e)
            ...
            if not input_text:
                ...
                def process_filter_file(uploaded_file):
                    try:
                        ...
                        logging.error("Error in submit filter: %s", e)
                with ThreadPoolExecutor() as executor:
